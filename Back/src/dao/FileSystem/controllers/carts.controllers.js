import { findCartById, insertCart, upCart, delCart } from '../../../services/FileSystem/carts.services.js'
import { findProductById } from '../../../services/FileSystem/products.services.js'
...
export default class CartManagerFS {
    ...
    createCart = async (req, res) => {
        ...
        try {
            ...
            if (!newCart) {
                ...
                req.logger.info(`Carrito creado con éxito - ID: ${newCart._id} - at ${new Date().toLocaleDateString()} / ${new Date().toLocaleTimeString()}`);
    ...
    updateQuantity = async (req, res) => {
        ...
        try {
            ...
            const productIndex = cart.products.findIndex(i => i.product === req.params.pid);
            if (productIndex === -1) {
                req.logger.warning(`No se encontró el producto - ID: ${req.params.pid}, Carrito ID: ${req.params.cid} - at ${new Date().toLocaleDateString()} / ${new Date().toLocaleTimeString()}`);
                return res.status(400).json({ Msg: `No se encontró el producto` });
            }
        ...
        try {
            ...
            for (const productItem of updateStock) {
                ...
                if (productItem.product.stock == 0) {
                    ...
                    await upProduct(productItem.product.id, { stock: productItem.product.stock, status: productItem.product.status });
                }
                await upProduct(productItem.product.id, { stock: productItem.product.stock });
            }
