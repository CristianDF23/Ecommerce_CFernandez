import { findCartById, insertCart, updateCart, deleteCart } from '../../../services/FileSystem/carts.services.js'
import { findProductById, updateProduct } from '../../../services/FileSystem/products.services.js'
...
export default class CartManagerFS {
    ...
    createCart = async (req, res) => {
        ...
        try {
            ...
            if (!isNewCart) {
                ...
                req.logger.info(`Carrito creado con éxito - ID: ${isNewCart._id} - at ${new Date().toLocaleDateString()} / ${new Date().toLocaleTimeString()}`);
    ...
    deleteProductCart = async (req, res) => {
        ...
        try {
            ...
            const existingItem = cart.products.findIndex(item => item.product === req.params.pid);
            if (existingItem === -1) {
            ...
            await updateCart(req.params.cid, cart);
    ...
    updateQuantityOfProduct = async (req, res) => {
        ...
        try {
            ...
            const productIndex = isCartFound.products.findIndex(i => i.product === req.params.pid);
            if (productIndex === -1) {
                req.logger.warning(`No se encontró el producto - ID: ${req.params.pid}, Carrito ID: ${req.params.cid} - at ${new Date().toLocaleDateString()} / ${new Date().toLocaleTimeString()}`);
                return res.status(400).json({ Msg: `No se encontró el producto` });
            }
        ...
        try {
            ...
            for (const productItem of updateStockProducts) {
                ...
                if (productItem.product.stock == 0) {
                    ...
                    await updateProduct(productItem.product.id, { stock: productItem.product.stock, status: productItem.product.status });
                }
                await updateProduct(productItem.product.id, { stock: productItem.product.stock });
            }
